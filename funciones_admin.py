...
def crear_tabla_editable(parent, headers, registros, tabla_sql, color_tabla="#e0e0e0", actualizar_callback=None, eliminar_callback=None, header_text_color=None, editar_callback=None, callback_recargar_tabla=None):
    """
    ...
    def mostrar_filas():
        ...
        for fila_idx, fila in enumerate(registros):
            row_widgets = []
            # Para Carreras y Materia, mostrar solo los campos visibles (saltando el ID en fila[0])
            # Para otras tablas, mostrar todos
            num_cols_mostrar = len(headers) if tabla_sql in ["Carreras", "Materia", "Grupo"] else len(fila)
            inicio_datos = 1 if tabla_sql in ["Carreras", "Materia", "Grupo"] else 0
            
            for col_idx in range(num_cols_mostrar):
                valor = fila[inicio_datos + col_idx]
            ...
            # BOTÓN ELIMINAR
            if eliminar_callback:
                # Usar una lambda que captura la referencia de la función de forma segura
                def hacer_eliminar(idx=fila_idx, id_registro=fila[0]):
                    def callback_eliminar():
                        try:
                            # Si hay callback para recargar tabla (recarga desde BD), usarlo
                            # Si no, solo redibuja con los registros actuales
                            if callback_recargar_tabla:
                                callback_recargar_tabla()
                            else:
                                mostrar_filas()
                        except:
                            # Si hay error, intenta recargar la tabla base
                            pass
...
def mostrar_dashboard(frame):
    ...
    catalogos = [
        ...
        ("Calif. Actividades",   "Gestión de calif. parciales",   lambda: mostrar_calificaciones_actividades(frame),   "#2D3250", icono_calificaciones),
...
def mostrar_carreras(frame):
    ...
    mostrar_seccion_gestion(frame, "Gestión de Carreras", "#43000E", "#ffffff", "#d1c4b3", botones, headers, tabla_sql="Carreras", header_text_color="white")
...
def mostrar_materias(frame):
    def registrar(area,volver):
        try:
            ...
            print(f"Error al mostrar formulario de materias: {e}")
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Error al mostrar formulario: {str(e)}")
